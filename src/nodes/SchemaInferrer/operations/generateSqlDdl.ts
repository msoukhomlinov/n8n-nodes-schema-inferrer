...
/**
 * Inject IF NOT EXISTS semantics into a single ALTER TABLE ADD COLUMN statement.
 * The raw SQL is generated by Knex's schema.table() builder.
 */
function addIfNotExistsToStatement(
  sql: string,
  columnName: string,
  tableName: string,
  databaseType: string,
  ...
  switch (databaseType) {
    ...
    case 'cockroachdb':
    case 'sqlite3':
      // PostgreSQL / CockroachDB / SQLite 3.37.0+
      // Knex produces: alter table "t" add column "col" <type>
      return sql.replace(/\badd column\b/i, 'add column if not exists');

    case 'mysql2':
      // MariaDB 10.0.2+ / MySQL 8.0+
      // Knex produces: alter table `t` add `col` <type>  (no "column" keyword)
      return sql.replace(/\badd\b(\s+)/i, 'add column if not exists$1');

    case 'mssql': {
      // MSSQL has no native ADD COLUMN IF NOT EXISTS — use sys.columns check
      return (
        `if not exists (\n` +
    ...
    case 'oracledb': {
      // Oracle: PL/SQL block — ORA-01430 means the column already exists
...
/**
 * Convert JSON Schema to a set of ALTER TABLE ADD COLUMN IF NOT EXISTS statements.
 * One statement is produced per top-level schema property.
 */
function convertSchemaToTopupSql(
  schema: JsonSchema,
  rootSchema: JsonSchema,
  tableName: string,
  databaseType: string,
  ...
  try {
    ...
    for (const [propertyName, propertyDef] of Object.entries(properties)) {
      ...
      let resolvedProperty = dereferenceSchema(propertyDef as JsonSchema, rootSchema);
      if (resolvedProperty.items && typeof resolvedProperty.items === 'object') {
        const itemsSchema = resolvedProperty.items as JsonSchema;
        if (isRefSchema(itemsSchema)) {
          resolvedProperty = { ...resolvedProperty, items: dereferenceSchema(itemsSchema, rootSchema) };
        }
      }
    ...
    // Oracle uses PL/SQL blocks separated by "/" terminators
    if (databaseType === 'oracledb') {
...
export function generateSqlDdl(
  ...
  try {
    ...
    const generateTopupQuery = context.getNodeParameter('generateTopupQuery', 0, false) as boolean;
    const json: IDataObject = { sql, tableName, databaseType };
    if (generateTopupQuery) {
      json.topupSql = convertSchemaToTopupSql(
        schema,
        rootSchemaWithDefinitions,
        tableName,
        databaseType,
